...
const Logo3D: React.FC<{ className?: string }> = ({ className = "" }) => (
  ...
  >
    ...
    <path
      d="M50 5 L70 20 L50 35 L30 20 Z"
      fill="#f43f5e"
      className="opacity-90"
    />{" "}
    {/* Head */}
    <path
      d="M30 25 L70 25 L85 55 L50 85 L15 55 Z"
      fill="#8b5cf6"
      className="opacity-90"
    />{" "}
    {/* Body */}
    <path
      d="M50 85 L15 55 L25 95 L50 95 Z"
      fill="#3b82f6"
      className="opacity-80"
    />{" "}
    {/* Leg Left */}
    <path
      d="M50 85 L85 55 L75 95 L50 95 Z"
      fill="#06b6d4"
      className="opacity-80"
    />{" "}
    {/* Leg Right */}
    ...
    <path
      d="M50 5 L50 35 M30 20 L70 20 M50 35 L50 85"
...
const App: React.FC = () => {
  ...
  // Row Heights configuration (Normal vs Print)
  // CHANGE THESE VALUES TO ADJUST CARD HEIGHTS MANUALLY
  const rowHeights = {
    desc: printMode ? "h-[10rem]" : "h-40", // Description Card
    stats: printMode ? "h-[14rem]" : "h-48", // Stats Chart Card
    comm: printMode ? "h-[14rem]" : "h-60", // Communication Card
    impact: printMode ? "h-[13rem]" : "h-48", // Impact Card
  };

  return (
    <div
      className={`min-h-screen bg-[#0f172a] text-slate-100 pb-20 print-canvas`}
    >
      {/* Navigation / Header - Hide some buttons in print */}
      <nav className="sticky top-0 z-40 bg-[#0f172a]/80 backdrop-blur-lg border-b border-white/5 px-6 py-4 flex justify-between items-center no-print">
        ...
        <div className="flex gap-3">
          ...
          {printMode ? (
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all bg-emerald-500 text-white hover:bg-emerald-400 shadow-lg shadow-emerald-500/25 animate-pulse"
            >
              <Download size={16} />
              Save PDF / Print
            </button>
          ...
          >
            <Printer size={16} />
            {printMode ? "Exit Poster Mode" : "Poster Mode"}
          </button>
      ...
      <main
        className={`container mx-auto px-4 mt-8 ${
          printMode ? "max-w-none w-full px-8" : "max-w-[1600px]"
        }`}
      >
        {/* Intro Text - INCLUDED IN PRINT NOW */}
        <div
          className={`text-center mb-16 max-w-5xl mx-auto space-y-8 ${
            printMode ? "py-8" : ""
          }`}
        >
        ...
        {/* The Grid - In print mode we ensure grid holds shape */}
        <div
          className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 ${
            printMode ? "print-grid gap-8" : ""
          }`}
        >
          {PERSONALITY_DATA.map((p: PersonalityData) => (
            <div
              ...
              className={`relative flex flex-col rounded-3xl overflow-hidden border ${
                p.borderColor
              } bg-[#1a2236] transition-all duration-500 break-inside-avoid ${
                !printMode ? "hover:shadow-2xl hover:scale-[1.01]" : ""
              } ${p.shadowColor}`}
            >
              {/* Header Section: 3D Figure & Title */}
              <div
                className={`p-6 pb-0 relative overflow-hidden min-h-[360px] flex flex-col pt-10`}
              >
                {/* Background Glow */}
                <div
                  className={`absolute -top-32 -right-32 w-96 h-96 rounded-full blur-[100px] ${p.bgColor} opacity-40`}
                />
                <div
                  className={`absolute -bottom-10 -left-10 w-64 h-64 rounded-full blur-[80px] ${p.bgColor} opacity-20`}
                />

                <div className="relative z-10 flex flex-col items-center flex-grow w-full">
                  {/* Title - Responsive size */}
                  <h3
                    className={`text-4xl md:text-5xl font-black uppercase mb-4 w-full text-center tracking-tight ${p.textColor} drop-shadow-lg`}
                  >
                    {p.type}
                  </h3>

                  {/* Tagline - Responsive size and wrapping for mobile */}
                  <div className={`w-full max-w-[90%] text-center`}>
                    <div
                      className={`inline-block text-[10px] md:text-xs font-bold tracking-widest uppercase mb-8 px-4 py-2 rounded-full bg-white/5 backdrop-blur-md border border-white/10 ${p.textColor} shadow-lg z-20 whitespace-normal leading-relaxed`}
                    >
                      {p.tagline}
                    </div>
                  </div>
              ...
              <div className="flex-1 p-4 pt-0 grid grid-cols-1 gap-4 z-20">
                  ...
                  back={
                    <div className="w-full h-full overflow-y-auto no-scrollbar">
                      <div className="min-h-full">
                        <h5
                          className={`text-xs uppercase font-bold mb-3 ${p.textColor} opacity-80 text-center sticky top-0 bg-slate-900/95 py-1 z-10`}
                        >
                          Communication Keys
                        </h5>
                        <ul className="space-y-1.5 pb-2">
                          {p.communicationTips.map((tip, i) => (
                            <li
                              key={i}
                              className="flex items-center gap-2 text-xs font-semibold text-slate-200 bg-white/5 p-1.5 rounded-lg border border-white/5"
                            >
                              <Target
                                size={12}
                                className={`shrink-0 ${p.textColor}`}
                              />
                ...
                <FlipCard
                  ...
                  back={
                    <div className="w-full h-full overflow-y-auto no-scrollbar text-left">
                      <div className="min-h-full">
                        <h5
                          className={`text-xs uppercase font-bold mb-3 ${p.textColor} opacity-80 text-center sticky top-0 bg-slate-900/95 py-1 z-10`}
                        >
                        ...
                        <div className="space-y-3 pb-2">
                          {p.impactOnOthers.map((impact, i) => (
                            <div
                              key={i}
                              className="text-xs border-l-2 border-slate-700 pl-2 leading-tight"
                            >
                              <span
                                className={`font-bold uppercase ${getImpactColor(
                                  impact.color
                                )}`}
                              >
                                {impact.color}:{" "}
              ...
              <div
                className={`h-2 w-full ${p.bgColor.replace(
                  "/10",
                  ""
                )} opacity-80`}
              ></div>
      ...
      <footer className="mt-20 text-center text-slate-600 text-sm pb-8 no-print">
        <p>
          © 2024 PersonaSpectrum. Concepts adapted from Jungian archetypes &
